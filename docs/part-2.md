# Part 2: Building your project as a Docker image
...
Save this file as `services/part-2/baby-steps/Dockerfile`. Let's now build the image. From the root of the repository, run:

```bash
docker build -t poetry-ws-part-2-baby-steps:latest -f services/part-2/baby-steps/Dockerfile .
```

And run it:

```bash
$ docker run --rm poetry-ws-part-2-baby-steps
 _________________
< somenergia.coop >
 -----------------
        \   ^__^
...
4. We build the image and tag it as `poetry-ws-part-2-baby-steps:latest`. The `-f` flag tells docker to use the `Dockerfile` at the given path.
5. We run the image. The `--rm` flag tells docker to remove the container after it has finished running. The `poetry-ws-part-2-baby-steps` is the name of the image we built. The `CMD` line tells docker to run `cowsay somenergia.coop` when the container starts.
...
services:
  baby-steps:
    image: poetry-ws-part-2-baby-steps:latest
    build: services/part-2/baby-steps
```

Save this file as `docker-compose.part-2.yml`. Let's now build the image. From the root of the repository, run:

```bash
docker compose -f docker-compose.part-2.yml build
```

And run it:

```bash
docker compose -f docker-compose.part-2.yml run baby-steps
```
...
```bash
$ docker run -it --rm poetry-ws-part-2-baby-steps bash
root@e5e06508458d:/#
```
...
```bash
docker compose -f docker-compose.part-2.yml run -it --rm baby-steps bash
```
...
2. Open the remote explorer and look for the image with the name `poetry-ws-part-2-baby-steps:latest`. Right click on it and select `Attach in current window`.

   ![Alt text](assets/images/part-2-attach-to-container.png)
...
3. Visual studio code will now run from inside the container. You can now edit files and run commands from the terminal. Check the left bottom corner to confirm it!
   ...
   ![Alt text](assets/images/part-2-vscode-inside-container.png)

## Building a python image

Let's now build a python image. We'll use the `python:3.9-slim` image as a base. Save this file as `services/part-2/basic-python/Dockerfile`.
...
Add a new service to the `docker-compose.part-2.yml` file:
...
services:
  baby-steps:
    ...
    build: services/part-2/baby-steps
  basic-python: # new
    image: poetry-ws-part-2-basic-python:latest # new
    build: services/part-2/basic-python # new
```

build the image and run it:

```bash
docker compose -f docker-compose.part-2.yml run  --build basic-python
```

## Recreating part 1 with docker
...
```bash
mkdir -p services/part-2/python-poetry
cd services/part-2/python-poetry
mkdir -p src/greeter
...
```bash
$ tree services/part-2/python-poetry
services/part-2/python-poetry/
├── Dockerfile
├── poetry.lock
├── pyproject.toml
├── README.md
...
```toml
# service/part-2/python-poetry/pyproject.toml
[tool.poetry]
name = "greeter" # new
version = "0.1.0"
description = ""
...
```python
# service/part-2/python-poetry/src/part_2/cli.py
import cowsay
...
```python
# service/part-2/python-poetry/src/part_2/__init__.py
```

```markdown
<!-- service/part-2/python-poetry/README.md -->
# Part 2
```
...
```dockerfile
# service/part-2/python-poetry/Dockerfile
# ---------------------------------------------------------------------------- #
...
services:
  baby-steps:
    ...
    build: services/part-2/baby-steps
  basic-python:
    image: poetry-ws-part-2-basic-python:latest
    build: services/part-2/basic-python
  python-poetry: # new 
    image: poetry-ws-part-2-python-poetry:latest # new
    build: services/part-2/python-poetry # new
```

build the image and run it:

```bash
docker compose -f docker-compose.part-2.yml run  --build python-poetry
```
...
```bash
docker compose -f docker-compose.part-2.yml push python-poetry
```
